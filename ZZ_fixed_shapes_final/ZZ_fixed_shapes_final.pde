...
class IncrCircle {
  ...
  IncrCircle (float r_, float x_, float y_, float g_, float b_, float d_, boolean fil_) {
    r = r_;
    x = x_;
    y = y_;
    g = g_;
...
void drawIncrCircle (IncrCircle f) {
  if (f.fil == true) {
    fill(f.r, f.g, f.b);
  }
...
//took this part below inspired by https://processing.org/examples/regularpolygon.html
void drawstrokePoly (strokePoly k) {
  stroke(k.r, k.g, k.b);
...
void drawfillCircle (fillCircle f) {
  if (f.fil) {
    fill(f.r, f.g, f.b);
  }
  ellipse (f.x, f.y, f.r, f.r);
